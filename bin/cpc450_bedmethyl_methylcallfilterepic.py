...
def main(file1_path, file2_path, output_path, methylation_output_path, filtered_output_path):
    ...
    # Step 3: Clean file2 by removing rows with "h" in the fourth column and reconstitute the DataFrame
    cleaned_rows = []
    with open(file2_path, 'r') as file2:
        reader = csv.reader(file2, delimiter='\t')  # Assuming tab-separated files
        header = next(reader)  # Read the header line
        for row in reader:
            if len(row) >= 4 and "h" not in row[3]:  # Exclude rows with "h" in column 4
                cleaned_rows.append(row)
                #print(cleaned_rows)

    # Step 4: Filter rows from cleaned file2 where the seventh column matches the "cgxxxxxxxx" pattern from file1
    matching_rows = []
    for row in cleaned_rows:
        if len(row) >= 7 and pattern.match(row[6]) and row[6] in pattern_elements:  # Column 7 (index 6) matches "cgxxxxxxxx" and is in pattern_elements
            matching_rows.append(row)  # Store the entire row

    # Step 5: Write the header and matching rows to the output file
    with open(output_path, 'w', newline='') as output_file:
        writer = csv.writer(output_file, delimiter='\t')
        writer.writerow(header)  # Write the header
        writer.writerows(matching_rows)  # Write matching rows

    # Step 6: Generate the methylation output file
    generate_methylation_file(output_path, methylation_output_path)

    # Step 7: Filter the methylation output file to remove rows with methylation_call -1
...
if __name__ == "__main__":
    if len(sys.argv) != 6:
        print("Usage: python script.py <file1> <file2> <output> <methylation_output> <filtered_output>")
        sys.exit(1)
