#!/usr/bin/env python3
"""
classify_sv_v5.py — Classify Sniffles SVs by gene-level impact using BEDTools.
...
v5: orders each fusion's gene_A/gene_B (and fusion_name) so gene_A is
    always the 5' partner and gene_B the 3' partner, inferred from each
    gene's strand and which side of the breakpoint is retained in the
    fused sequence (derived from SV type for intrachromosomal events, and
    from the BND ALT bracket notation for translocations). Falls back to
    the original left/own-vs-right/mate order when orientation is
    ambiguous (e.g. both partners resolve to the same end).
...
def parse_bnd_alt(alt):
    """Parse a VCF BND ALT field into its full join semantics.

    Returns (mate_chrom, mate_pos, own_retained, mate_retained, mate_flip),
    or all-None if alt doesn't match one of the four canonical BND forms.

    The four forms (VCF spec) and what they mean for which side of each
    breakpoint is retained in the fused sequence, and whether the mate
    piece is reverse-complemented when joined:

        t[p[  own retained = upstream (<=POS),   mate retained = downstream (>=p), no flip
        t]p]  own retained = upstream (<=POS),   mate retained = upstream (<=p),   flip
        ]p]t  own retained = downstream (>=POS), mate retained = upstream (<=p),   no flip
        [p[t  own retained = downstream (>=POS), mate retained = downstream (>=p), flip

    own is never flipped — it's always read in its native reference
    orientation; only the mate piece's effective strand can be reversed.
    """
    m = re.match(r'^([^\[\]]*)([\[\]])([^:]+):(\d+)([\[\]])([^\[\]]*)$', alt)
    if not m:
        return None, None, None, None, None
    prefix, bracket, mate_chrom, mate_pos, _bracket2, suffix = m.groups()
    mate_pos = int(mate_pos)
    if prefix and not suffix:
        own_retained = "upstream"
        mate_retained = "downstream" if bracket == "[" else "upstream"
        mate_flip = (bracket == "]")
    elif suffix and not prefix:
        own_retained = "downstream"
        mate_retained = "upstream" if bracket == "]" else "downstream"
        mate_flip = (bracket == "[")
    else:
        return None, None, None, None, None
    return mate_chrom, mate_pos, own_retained, mate_retained, mate_flip


def _is_five_prime(retained_side, strand):
    """Does the retained side of a gene correspond to its 5' end?

    retained_side: "upstream" (lower genomic coordinate) or "downstream".
    strand: "+" or "-".

    + strand: TSS at the low-coordinate end, so "upstream" retained = 5'.
    - strand: TSS at the high-coordinate end, so "downstream" retained = 5'.
    """
    return (retained_side == "upstream") != (strand == "-")


def order_five_to_three(partner_a, partner_b):
    """Order two fusion partners 5'-end first, 3'-end second.

    Each partner is a dict with at least keys "side" (retained "upstream"
    or "downstream") and "strand" ("+"/"-"/"unknown"); any other keys
    (gene, chrom, pos, element, gene_type, ...) are carried through
    unchanged in the returned dicts.

    Falls back to the original (a, b) order when orientation is
    ambiguous — both partners resolve to the same end (e.g. both look
    5'), which can happen with unknown strand or unusual topologies.
    """
    five_a = _is_five_prime(partner_a["side"], partner_a["strand"])
    five_b = _is_five_prime(partner_b["side"], partner_b["strand"])
    if five_b and not five_a:
...
def detect_fusions(bp_bed, bnd_context, exons_detail_bed, genes_bed,
    ...
    Two sources:
      intrachromosomal — DEL/DUP/INV with one breakpoint inside a gene
                         (candidate gene A) and the other inside a
                         different gene (candidate gene B).
      translocation    — paired BND records where both breakpoints fall
                         inside distinct genes (mates resolved via ALT field).

    Gene A/B ordering: gene_A is always the inferred 5' partner and
    gene_B the 3' partner (fusion_name = "geneA::geneB"), determined from
    each gene's strand and which side of its breakpoint is retained in
    the fused sequence — derived from SV type for intrachromosomal events
    (DUP joins the duplicated segment's inside edges; DEL/INV/other join
    the outside flanks) and from the BND ALT bracket notation for
    translocations. Falls back to the candidate's original left/own vs.
    right/mate position when orientation is ambiguous.
    ...
    # Build gene_name → gene_type and gene_name → strand lookups from genes_bed
    # genes.bed name col = "gene_name|gene_id|gene_type", strand = BED col 6
    _biotype = {}
    _strand = {}
    ...
    for sv_id, data in sv_bp.items():
        ...
        for ga, ea in left_genes:
            for gb, eb in right_genes:
                ...
                # Which side of the breakpoint is retained in the fused
                # sequence depends on SV type: a tandem duplication joins
                # the *inside* edges of the duplicated segment (gene A's
                # downstream part to gene B's upstream part — validated
                # against the canonical FGFR3(5')::TACC3(3') duplication
                # fusion); deletions/inversions/others join the outside
                # flanks (gene A's upstream part to gene B's downstream
                # part).
                if data["svtype"] == "DUP":
                    side_a, side_b = "downstream", "upstream"
                else:
                    side_a, side_b = "upstream", "downstream"

                partner_a = {"gene": ga, "element": ea, "gene_type": ta,
                             "chrom": data["chrom"], "pos": left_pos,
                             "side": side_a, "strand": _strand.get(ga, "unknown")}
                partner_b = {"gene": gb, "element": eb, "gene_type": tb,
                             "chrom": data["chrom"], "pos": right_pos,
                             "side": side_b, "strand": _strand.get(gb, "unknown")}
                p5, p3 = order_five_to_three(partner_a, partner_b)

                fusions.append([
                    sv_id, f"{p5['gene']}::{p3['gene']}", data["svtype"], "intrachromosomal",
                    p5["chrom"], p5["pos"], p5["gene"], p5["element"], p5["gene_type"],
                    p3["chrom"], p3["pos"], p3["gene"], p3["element"], p3["gene_type"],
    ...
    for bnd_id, rec in bnd_meta.items():
        ...
        # Each gene's 5'/3' status depends only on which side of its own
        # breakpoint is retained (upstream/downstream) and its own native
        # strand — NOT on whether the mate piece is joined reverse-
        # complemented. Reverse-complementing a segment for assembly does
        # not change which of the gene's own exons it contains; applying
        # mate_flip to the mate's strand here previously made the result
        # depend on which of the two reciprocal BND records (own vs mate)
        # happened to be processed first, giving a different fusion_name
        # for the exact same physical junction. Using native strand only
        # is deterministic and agrees across both reciprocal records.
        # Fall back to the original upstream/downstream split if the ALT
        # field didn't match one of the four canonical BND forms.
        own_side = rec.get("own_retained") or "upstream"
        mate_side = rec.get("mate_retained") or "downstream"

        partner_own = {"gene": ga, "element": ea, "gene_type": ta,
                       "chrom": rec["chrom"], "pos": str(rec["pos"] - 1),
                       "side": own_side, "strand": _strand.get(ga, "unknown")}
        partner_mate = {"gene": gb, "element": eb, "gene_type": tb,
                         "chrom": rec["mate_chrom"], "pos": str(rec["mate_pos"] - 1),
                         "side": mate_side, "strand": _strand.get(gb, "unknown")}
        p5, p3 = order_five_to_three(partner_own, partner_mate)

        fusions.append([
            bnd_id, f"{p5['gene']}::{p3['gene']}", "BND", "translocation",
            p5["chrom"], p5["pos"], p5["gene"], p5["element"], p5["gene_type"],
            p3["chrom"], p3["pos"], p3["gene"], p3["element"], p3["gene_type"],
            "yes" if ea.startswith("exon_") and eb.startswith("exon_") else "no",
            "yes" if ta == "protein_coding" and tb == "protein_coding" else "no",
...
def build_bnd_context(bnd_bed, exons_detail_bed, genes_bed, vcf, window=0):
    ...
    bnd_meta   : {bnd_id: {chrom, pos, mate_chrom, mate_pos,
                            own_retained, mate_retained, mate_flip}}
    ...
    for line in raw.splitlines():
        ...
        try:
            ...
            continue
        mc, mp, own_retained, mate_retained, mate_flip = parse_bnd_alt(alt)
        bnd_meta[bnd_id] = {"chrom": chrom, "pos": pos_i,
                             "mate_chrom": mc or ".", "mate_pos": mp,
                             "own_retained": own_retained,
                             "mate_retained": mate_retained,
                             "mate_flip": mate_flip}
